...
If capital adjusts instantly, the timing on the Euler equation changes, there is no expectational error. There are still 3 dependent variables in the Euler equation: $\check{L}_{t}$, $\check{I}_{t}$, and $\check{K}_{t}$, so the Euler equation is still estimable, allowing one to obtain an estimate for $a$ as before.
