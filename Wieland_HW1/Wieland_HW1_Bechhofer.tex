...
For the right hand side, we collect terms to define
\[
\bar{x} = \alpha (1-\tau) \bar{K}_{t+1}^{\alpha-1} \bar{L}_{t+1}^{1-\alpha} + \delta^2 a + (1-\delta)(1+\delta a)
\]
and we get 
\[
\frac{2 \delta a}{1 + \delta a} (\check{I}_t - \check{K}_t) = \bar{x}^{-1} \alpha (1-\alpha) (1-\tau) (\bar{L}/\bar{K})^{1-\alpha} E[\check{L}_{t+1} - \check{K}_{t+1}] + 2 \bar{x}^{-1} \alpha \delta E[\check{I}_{t+1} - \check{K}_{t+1}] - \frac{\bar{r}}{1+ \bar{r}} E[\check{r}_{t+1}]
\]
