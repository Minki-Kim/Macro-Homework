...
\begin{enumerate}[label = (\alph*)]
	\item 
	However, convex cost can be calibrated to match the magnitude of the price changes Large price changes are followed by increasing costs, so we can calibrate for magnitudes, but not frequency (since no fixed cost implies prices change continuously).
	...
	\item
	The random walk assumption implies $\mathbb{E}_{t} [m_{t+l}] = m_t$ for all $l \geq 0$ and we have
	\begin{equation*}
...
\begin{enumerate}[label = (\alph*)]
	\item The optimal reset price is
	\begin{equation*}
	...
	\end{equation*}
	Substitutie all variables which are defined with respect to time
	\begin{align*}
	...
	\end{align*}
	Price level is indeterminate so we substitute 
	\begin{align*}
	...
	\end{equation*}
	${\pi_t^*}^{\frac{1-\alpha + \sigma \alpha}{1-\alpha}}$ is the \textit{reset price inflation} rate. 
	Use this expression in defining inflation and reset price in the Dynare code for (b) and (c).
	...
	\item  Calibrated impulse responses are close to the given responses qualitatively and quantitatively.
	\begin{figure}[H]
		\centering
...
\begin{enumerate}[label = (\alph*)]
	...
	\end{align*}
	The first equation is a standard Euler equation. 
	The second equation is a recursive formulation of inflation rate, telling us that current inflation is a present value of future marginal costs. 
	The third equation is household's labor supply. 
	The fourth equation denotes aggregate production function. 
	The fifth equation is national account, where $s_g$ is the share the government spending. 
	Finally, the last equation implies that the central bank follows the Taylor rule. 
	...
	$1$ is not a state variable because the model is log-linearized. 
	Since $\check{ C }_{t-1}$ and $\pi_{t-1}$ do not show up in the system, lagged endogenous variables are also not state variables. 
	
	\item In our model, the minimum set of state variables is $\{\check{ G }_t\}$. The solution equations for $\check{ C }_t$ and $\pi_t$ are expressed as functions of state variable $\check{ G }_t$:
	\begin{align*}
	...
	\end{align*}
	\item Following the solution from above we have 
	\begin{align*}
	...
	\end{align*}
	\item Since $\left( 1 -  \beta \rho \right) ( 1- \rho ) + \kappa(\phi_\pi - \rho) >1$, $	dY / dG  < 1$. 
	The government spending multiplier is smaller than one because of the offset from monetary policy.
	...
	& > 1
	\end{align*}
	So the government spending multiplier is larger than 1 even for a very low persistence shock. 
	This is because the monetary policy is not raising nominal interest rate enough to make the real interest rate $r_t = i_t - E_t \pi_{t+1}$ also rise. 
	Since the real interest rate is too low, even a small demand shock can make a ripple effect. 
	...
	\end{align*}
	\item Following the solution obtained in (i), 
	\begin{align*}
	\frac{d Y}{d G} =  1 + \frac{d C}{d G} &= 1 + \frac{1-s_g}{s_g} \frac{d \check{ C }}{d \check{ G }} \\
	...
	\end{align*}
	\item Given $( 1- \beta p ) ( 1- p ) - \frac { \kappa } { \psi } \left[ \psi + \frac { 1} { \eta } \left( 1- s _ { g } \right) \right] p > 0$, the government spending multiplier is always larger than 1. 
	This is because the Taylor rule is not working at the ZLB, so the monetary policy does not stabilize the economy. 
	...
	\end{align*}
	Those two multipliers are the same when $p = \rho$. 
	In (g), monetary policy is ineffective indefinitely. 
	Thus the persistence of the government spending shock determines the size of the multiplier. 
	In (k), on the other hand, the probability of escaping form the ZLB determines the size of the multiplier, since $\rho=1$ as long as the economy is trapped in the ZLB. 
\end{enumerate}
