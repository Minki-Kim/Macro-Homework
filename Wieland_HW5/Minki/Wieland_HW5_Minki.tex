...
\[
\min_{x_t} \frac{1}{2} \times (x_t - (p_t + \phi y_t))^2 + \sum_{j=1}^{\infty} \beta^j (1-\alpha)^j  \frac{1}{2} E_t \left( x_t + \sum_{k=0}^{j-1} \gamma \pi_{t+k} - \left[ p_t + \sum_{k=1}^j \pi_{t+k} + \phi y_{t+k} \right] \right)^2
\]
...
The profit maximizing price in the future is accounted for with the $p_t + \sum_{k=1}^j \pi_{t+k} + \phi y_{t+k}$ term.

Now we solve the firm's problem using the first order condition
\[
x_t - (p_t + \phi y_t) + \sum_{j=1}^{\infty} \beta^j (1-\alpha)^j E_t \left( x_t + \sum_{k=0}^{j-1} \gamma \pi_{t+k} - \left[ p_t + \sum_{k=1}^j \pi_{t+k} + \phi y_{t+k} \right] \right) = 0
\]
...
\[
\left[ \sum_{j=0}^{\infty} [\beta \times (1-\alpha)]^j x_t - p_t \right] + \phi y_t +  \sum_{j=1}^{\infty} \beta^j (1-\alpha)^j E_t \left( \sum_{k=0}^{j-1} \gamma \pi_{t+k} - \left[ \sum_{k=1}^j \pi_{t+k} + \phi y_{t+k} \right] \right) = 0
\]
so we can use the formula for an infinite geometric series to get
\[
\frac{x_t - p_t}{1 - \beta (1-\alpha)} + \phi y_t +  \sum_{j=1}^{\infty} \beta^j (1-\alpha)^j E_t \left( \sum_{k=0}^{j-1} \gamma \pi_{t+k} - \left[ \sum_{k=1}^j \pi_{t+k} + \phi y_{t+k} \right] \right) = 0
\]
