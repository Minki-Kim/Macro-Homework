...
\section{$q$-Theory with Variable Capital Utilization}
    ...
    \begin{enumerate}
        ...
        \item Firms will invest (divest) if $q$ is greater (less) than unity. Hence procyclicality of investment is tightly linked to procyclicality of $q$. This model predicts that utilization is \emph{countercyclical} since $U$ is inversely related to $q$. Cyclicality, however, also depends on

        % other factors may be driving investment cyclicality than return to capital:
        % GE considerations: Y through L
        % depends on the underlying shocks: G, Z, demand
        % how do adjustment costs change things?

        \item Consider a positive productivity shock.
